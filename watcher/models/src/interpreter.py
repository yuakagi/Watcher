...
class WatcherInterpreter(object):
    ...
    def translate_numerics(self, lab_codes: np.ndarray, numerics: np.ndarray):
        ...
        if config.NUM_STRATEGY == "percentile":
            # <--- Strategy using percentile --->
            num_df = pd.merge(
                num_df, self.percentiles, on=config.COL_ITEM_CODE, how="left"
            )
            mask = (
                num_df[self.percentile_cols + [config.COL_NUMERIC]].notna().all(axis=1)
            )
            ...
            # <--- Strategy using z-score --->
            num_df = pd.merge(
                num_df,
                self.num_stats[
                    [config.COL_ITEM_CODE, "unit", "mean", "std", "min", "max"]
                ],
                on=config.COL_ITEM_CODE,
                how="left",
            )
            mask = num_df[["mean", "std", config.COL_NUMERIC]].notna().all(axis=1)
            num_series = num_df.loc[mask, config.COL_NUMERIC].values
            means = num_df.loc[mask, "mean"].values
            stds = num_df.loc[mask, "std"].values
            upper = num_df.loc[mask, "max"].values
            lower = num_df.loc[mask, "min"].values
            target_vals = (num_series * stds) + means
            target_vals = np.clip(target_vals, lower, upper)
