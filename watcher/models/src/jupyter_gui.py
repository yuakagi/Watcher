    ...
    def interpret_predicted_indexes(self, indexes: list, lab_code: str = None) -> list:
        ...
        if lab_code is not None:
            ...
            if config.NUM_STRATEGY == "z-score":
                df = pd.merge(
                    df,
                    self.model.interpreter.num_stats,
                    on=[config.COL_ITEM_CODE],
                    how="left",
                )
                df["n"] = pd.to_numeric(df[config.COL_ORIGINAL_VALUE], errors="coerce")
                num_mask = (
                    (df["n"].notna()) & (df["mean"].notna()) & (df["std"].notna())
                )
                df.loc[num_mask, "unscaled"] = (
                    (
                        df.loc[num_mask, "n"] * df.loc[num_mask, "std"]
                        + df.loc[num_mask, "mean"]
                    )
                    .round(5)
                    .astype(str)
                )
    ...
    def _demo_retrospective(
        ...
        if self.display_numeric:
            ...
            # <-- z-score -->
                ...
                ax2.set_xlabel("Z-score", fontsize=FONT_S)
        ...
        if self.display_numeric:
            ...
            # <-- z-score -->
                ...
                stem_xs = compute_zscore_centers(num_bins=self.model.n_numeric_bins)
                ax2.set_xlim(config.NUM_LOWER_BOUND, config.NUM_UPPER_BOUND)
