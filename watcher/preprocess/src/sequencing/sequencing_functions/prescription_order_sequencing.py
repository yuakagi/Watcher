"""Module to align prescription orders (except for injections)"""
...
def _sequence_prescription_order_records(
    df: pd.DataFrame,
    task_no: int,
    categorical_dim: int,
    med_token_map: pd.DataFrame,
) -> pd.DataFrame:
    """Helper function that performs preprocessing steps for prescription order records.

    Args:
        df (pd.DataFrame): Dataframe to be processed.
        task_no (int): Number of current task processed by the child process. This is passed by 'parallel_map_partition' function.
...
def sequence_prescription_order_records() -> dict:
    """Aligns records linearly before creating patient timeline matrices to prescription order records.
    ...
    output_file_path = os.path.join(
        ...
        config.PRESCRIPTION_ORDER_TABLE_PATTERN.replace("*", "sequenced"),
    )
    ...
    stats_list = parallel_map_partitions(
        ...
        function=_sequence_prescription_order_records,
...
def sequence_prescription_order_records_single(
    df: pd.DataFrame, categorical_dim: int, med_token_map: pd.DataFrame
) -> pd.DataFrame:
    ...
    """
    if df.size:
        df, _ = _sequence_prescription_order_records(
            df=df,
            task_no=0,
            med_token_map=med_token_map,
            categorical_dim=categorical_dim,
