...
def _sequence_lab_result_records(
        ...
        for i in range(0, len(unique_patients), patient_batch_size):
            ...
            if p_num_mask.any():
                ...
                if percentiled.any():
                    ...
                    vals = num_df.loc[percentiled, config.COL_NUMERIC].values.reshape(
                        -1, 1
                    )
    ...
    # <--- Normalization using z-score --->
        ...
        df = pd.merge(
            df,
            num_stats[[config.COL_ITEM_CODE, "unit", "mean", "std"]],
            on=[config.COL_ITEM_CODE, "unit"],
            how="left",
        )
        df["std"] = df["std"].clip(lower=1e-7)  # Avoid zerodivisionerror
        df[config.COL_NUMERIC] = (df[config.COL_NUMERIC] - df["mean"]) / df["std"]
        df = df.drop(["mean", "std"], axis=1)

        # Clip values
        df[config.COL_NUMERIC] = df[config.COL_NUMERIC].clip(
            lower=config.NUM_LOWER_BOUND, upper=config.NUM_UPPER_BOUND
        )
