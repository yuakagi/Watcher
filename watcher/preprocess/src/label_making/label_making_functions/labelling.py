...
def label_numerics(
    ...
    # <--- Labeling using z-score --->
    else:
        # NOTE: Outliers are included in the first/last bins
        step_size = (config.NUM_UPPER_BOUND - config.NUM_LOWER_BOUND) / (
            n_numeric_bins - 1
        )
        nums = df.loc[num_mask, config.COL_NUMERIC].values
        nums = np.clip(nums, a_min=config.NUM_LOWER_BOUND, a_max=config.NUM_UPPER_BOUND)
        diff_from_min = nums - config.NUM_LOWER_BOUND
        numeric_labels = np.floor(diff_from_min / step_size) + first_numeric_label
        df.loc[num_mask, config.COL_LABEL] = numeric_labels
