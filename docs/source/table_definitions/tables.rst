...
Please prepare all the CSV files listed below. These are uploaded into the PostgreSQL database.
If the csv name contains a wildcard (*), it indicats that files can be split into multiple files.
Make sure all the CSV files are in the same directory.
